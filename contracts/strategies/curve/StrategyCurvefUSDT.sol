...
contract StrategyCurvefUSDT is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address constant public want = 0x92D5ebF3593a92888C25C0AbEF126583d4b5312E; // curve lpToken
    ...
    address constant public token2pool = 0x27E611FD27b276ACbd5Ffd632E5eAEBEC9761E40;
    address constant public fusdt = 0x049d68029688eAbF473097a2fC38ef61633A3C7A;
    ...
    address constant public rewardsGauge = 0x06e3C4da96fd076b97b7ca3Ae23527314b6140dF;
    address constant public pool = 0x92D5ebF3593a92888C25C0AbEF126583d4b5312E;

    // Routes
    address[] public crvToNativeRoute;
    address[] public nativeTofusdt;
    address[] public nativeTotoken2pool;
    ...
    constructor(
        ...
        nativeTofusdt = [native, fusdt];
        nativeTotoken2pool = [native, token2pool];
    ...
    function addLiquidity() internal {
        ...
        //Calculate amount of USDC+DAI in the pool
        uint256 balancePooltoken2pool = IERC20(token2pool).balanceOf(pool).mul(ICurveSwap2(token2pool).get_virtual_price()).div(1e18);

        //Calculate amonut of FUSDT in the pool
        uint256 balancePoolfusdt = IERC20(fusdt).balanceOf(pool).mul(1e18).div(1e6);

        //Deposit the token that has the lowest amount
        //If amount of tokens in token2pool is smaller, then we add liquidity to the Curve 2pool
        uint256 depositBal;
        uint8 depositIndex;

        if (balancePooltoken2pool > balancePoolfusdt) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(nativeBal, 0, nativeTofusdt, address(this), block.timestamp);
            depositBal = IERC20(fusdt).balanceOf(address(this));
            depositIndex = 0;
        } else {
            uint256 balancePoolUSDC = IERC20(usdc).balanceOf(token2pool).mul(1e18).div(1e6);
            uint256 balancePoolDAI = IERC20(dai).balanceOf(token2pool);

            (address depositToken, 
            uint256 depositIndex2pool, 
            ...
            uint256 depositBal2pool = IERC20(depositToken).balanceOf(address(this));
            

            uint256[2] memory amounts2pool;
            amounts2pool[depositIndex2pool] = depositBal2pool;
            ICurveSwap2(token2pool).add_liquidity(amounts2pool, 0);
            
            depositBal = IERC20(token2pool).balanceOf(address(this));
            depositIndex = 1;
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(fusdt).approve(pool, type(uint).max);
        IERC20(token2pool).approve(pool, type(uint).max);
        IERC20(usdc).approve(pool, type(uint).max);
        IERC20(dai).approve(token2pool, type(uint).max);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(fusdt).approve(pool, 0);
        IERC20(token2pool).approve(pool, 0);
        IERC20(usdc).approve(token2pool, 0);
        IERC20(dai).approve(token2pool, 0);
