...
abstract contract VaultBase is IVault, Ownable, ReentrancyGuard {
    ...
    uint256 public override withdrawPenaltyTime = 0 days;
    // Withdrawal penalty, 100 = 1%
    uint256 public override withdrawPenalty = 0;
    // For vaults that are farming pools with a deposit fee
    uint256 public depositFee = 0;
    //Allowed amount of the token sent to the fee dist each vault can mint rewards for, default 1000 (1000 WMATIC = roughly 0.65 ETH = 312 )
    uint256 public override rewardAllocation = 0;//1e18 * 1000;

    // We should not use boosting
    // Certain vaults will give up to 10x in rewards
    // Additional usecase: lock it to boost the yield of a certain vault
    ...
    //Increase the amount of the token sent to the fee dist the vault is allowed to mint for
